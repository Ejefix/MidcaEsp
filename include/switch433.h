...
public:
  Switch433();
  void set_pair_signal(const std::pair<size_t, size_t> &sig);
  void remove_pair_signal(const std::pair<size_t, size_t> &sig);
  void set_search(bool activ);
  void set_pin_duwn(size_t pin_);
  void set_pin_up(size_t pin_);
...
private:
  ...
  RCSwitch mySwitch;
  bool search{true};
  std::set<size_t> signals_new;
  std::vector<std::pair<size_t, size_t>> signalsUP;
