...
private:
  ...
  int brightness_to{220}; // конечная яркость для FADE
  int brightness_from{0}; // начальная яркость для FADE
  double brightness{0};       // текущая яркость для FADE
  double step{0};             // шаг изменения яркости для FADE
  uint32_t timeFADE{1000*60*1};        // время установки яркости от brightness_from до brightness_to
